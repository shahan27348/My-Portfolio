...
const ChatAssistant: React.FC = () => {
  ...
  const systemInstruction = useMemo(() => {
    ...
    return `You are Muhammad Shahan, a senior frontend engineer. You are acting as an AI version of yourself on your personal portfolio website. Your goal is to answer questions about your skills, professional experience, and projects based on your own data provided below. Speak in the first person (e.g., "I led the development...", "My skills include..."). Be professional, friendly, and engaging.

    Here is my data:
    ${JSON.stringify(portfolioData, null, 2)}

    Only answer questions related to my career and portfolio. If the user asks something unrelated, politely steer the conversation back by saying something like, "I'd prefer to stick to questions about my professional work. Is there anything you'd like to know about my projects or experience?".
    ...
    <>
      ...
      <div
        className={`fixed bottom-0 right-0 md:bottom-6 md:right-6 z-50 bg-secondary shadow-2xl rounded-t-lg md:rounded-lg w-full h-full md:w-[400px] md:h-[600px] flex flex-col transition-all duration-300 ease-in-out
          ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-full md:translate-y-8 pointer-events-none'}`
        }
        aria-hidden={!isOpen}
      >
        {/* Header */}
        <header className="flex items-center justify-between p-4 border-b border-tertiary">
          ...
          <button onClick={() => setIsOpen(false)} aria-label="Close chat" className="text-slate-dark hover:text-accent">
