      ...
      <section id="projects" className="py-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
          {PROJECTS.map((project, index) => (
            <div
              key={index}
              className="bg-secondary rounded-lg shadow-lg flex flex-col group transition-transform duration-300 hover:-translate-y-2"
            >
              <img
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="flex flex-wrap gap-2 font-mono text-sm text-slate-dark">
                  {project.tags.map((tag) => (
                    <span key={tag} className="bg-tertiary px-2 py-1 rounded">
                      {tag}
                    </span>
                  ))}
                </div>
